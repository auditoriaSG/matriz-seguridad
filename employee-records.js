...
const stagedRecordFiles=new Map();
...
function recordCompleteness(types){const required=types.filter(type=>type.codigo!=='OTRO'),completed=required.filter(type=>recordTypeDocuments(type.codigo).length>0).length,total=required.length;return {completed,total,percent:total?Math.round((completed/total)*100):0,complete:total>0&&completed===total}}
function renderEmployeeRecordDialog(employee,loading=false){
  ...
  const types=activeRecordData.tipos||[],completeness=recordCompleteness(types);
  target.innerHTML=`
    ...
    <section class="record-document-section"><div class="record-document-intro"><div><h3>Documentación</h3><p>Los documentos se almacenan de forma privada. Formatos permitidos: PDF, JPG, PNG o HEIC; máximo 10 MB.</p></div><span class="status-pill">${(activeRecordData.documentos||[]).length} archivo${(activeRecordData.documentos||[]).length===1?'':'s'}</span></div><div class="record-completeness ${completeness.complete?'complete':''}"><div><strong>${completeness.complete?'Expediente completo':'Expediente en proceso'}</strong><span>${completeness.completed} de ${completeness.total} documentos requeridos · “Otro documento” no cuenta</span></div><b>${completeness.percent}%</b><progress value="${completeness.completed}" max="${completeness.total||1}"></progress></div>
      <div class="record-document-grid">${types.map(type=>{const documents=recordTypeDocuments(type.codigo);return `<article class="record-document-card"><div class="record-document-card-heading"><h4>${safe(type.nombre)}</h4><span>${documents.length?`${documents.length} archivo${documents.length===1?'':'s'}`:'Pendiente'}</span></div><div class="record-file-list">${documents.length?documents.map(document=>`<button class="record-file" data-download-document="${safe(document.ruta_storage)}" title="Descargar ${safe(document.archivo_nombre)}"><span>${safe(document.archivo_nombre||'Documento')}</span><small>${safe(recordBytes(document.tamano_bytes))} · Descargar</small></button>`).join(''):'<p>Sin archivo cargado.</p>'}</div><div class="record-upload-actions"><label class="record-upload"><span>Seleccionar archivo</span><input type="file" data-record-upload="${safe(type.codigo)}" accept="application/pdf,image/jpeg,image/png,image/heic,.pdf,.jpg,.jpeg,.png,.heic"></label></div><small class="record-selected-file" data-selected-file="${safe(type.codigo)}">Ningún archivo seleccionado.</small><div class="record-file-preview" data-file-preview="${safe(type.codigo)}" hidden></div></article>`}).join('')}</div><div class="record-save-all"><span data-staged-count>${stagedRecordFiles.size?`${stagedRecordFiles.size} documento${stagedRecordFiles.size===1?'':'s'} listo${stagedRecordFiles.size===1?'':'s'} para guardar`:'Selecciona los documentos que deseas integrar.'}</span><button class="button primary" data-save-all-documents ${stagedRecordFiles.size?'':'disabled'}>Guardar todos los documentos</button></div>
    </section>`;
}
async function openEmployeeRecord(employeeId){
  ...
  stagedRecordFiles.clear();activeRecordEmployeeId=employeeId;activeRecordData={tipos:[],documentos:[]};
...
function updateSaveAllDocuments(){const button=$('[data-save-all-documents]'),count=$('[data-staged-count]');if(button){button.disabled=!stagedRecordFiles.size;button.textContent=stagedRecordFiles.size?`Guardar todos los documentos (${stagedRecordFiles.size})`:'Guardar todos los documentos'}if(count)count.textContent=stagedRecordFiles.size?`${stagedRecordFiles.size} documento${stagedRecordFiles.size===1?'':'s'} listo${stagedRecordFiles.size===1?'':'s'} para guardar`:'Selecciona los documentos que deseas integrar.'}
async function saveAllEmployeeDocuments(){
  const employee=(adminData.empleados||[]).find(item=>item.id===activeRecordEmployeeId);if(!employee||!stagedRecordFiles.size)return;
  const button=$('[data-save-all-documents]');if(button){button.disabled=true;button.textContent='Guardando documentos…'}
  const failures=[];
  for(const [typeCode,file] of [...stagedRecordFiles]){
    const path=`${employee.id}/${Date.now()}-${safeStorageName(file.name)}`;
    const uploaded=await appCore.records.upload(path,file);
    if(uploaded.error){failures.push(typeCode);continue}
    const saved=await appCore.records.register({employeeId:employee.id,typeCode,path,fileName:file.name,mimeType:file.type,size:file.size});
    if(saved.error){failures.push(typeCode);continue}
    stagedRecordFiles.delete(typeCode);
  }
  if(failures.length){updateSaveAllDocuments();toast(`No se pudieron guardar ${failures.length} documento${failures.length===1?'':'s'}. Inténtalo de nuevo.`);return}
  toast('Todos los documentos seleccionados se guardaron correctamente');await openEmployeeRecord(employee.id);
}
...
$('#employeeRecordDialog').onclick=event=>{if(event.target.closest('#closeEmployeeRecord')){$('#employeeRecordDialog').close();return}if(event.target.closest('[data-save-all-documents]')){saveAllEmployeeDocuments();return}const download=event.target.closest('[data-download-document]');if(download){appCore.records.download(download.dataset.downloadDocument).then(({data,error})=>{if(error||!data){toast('No se pudo preparar la descarga');return}window.open(data,'_blank','noopener')})}};
function updateRecordPreview(card,file){const preview=card?.querySelector('[data-file-preview]');if(!preview)return;const previous=preview.dataset.previewUrl;if(previous)URL.revokeObjectURL(previous);delete preview.dataset.previewUrl;preview.hidden=true;preview.innerHTML='';if(!file)return;const url=URL.createObjectURL(file);preview.dataset.previewUrl=url;preview.hidden=false;if(file.type==='application/pdf'){preview.innerHTML=`<p>Vista previa del PDF</p><iframe src="${url}" title="Vista previa de ${safe(file.name)}"></iframe>`;return}if(['image/jpeg','image/png'].includes(file.type)){preview.innerHTML=`<p>Vista previa de la imagen</p><img src="${url}" alt="Vista previa de ${safe(file.name)}">`;return}preview.innerHTML=`<p>Archivo HEIC seleccionado. Se guardará correctamente; su vista previa dependerá del navegador.</p>`}
$('#employeeRecordDialog').onchange=event=>{const input=event.target.closest('[data-record-upload]');if(!input)return;const file=input.files?.[0],card=input.closest('.record-document-card'),message=card?.querySelector('[data-selected-file]'),allowed=['application/pdf','image/jpeg','image/png','image/heic'];if(!file){stagedRecordFiles.delete(input.dataset.recordUpload);if(message)message.textContent='Ningún archivo seleccionado.';updateRecordPreview(card,null);updateSaveAllDocuments();return}if(!allowed.includes(file.type)||file.size>10485760){input.value='';stagedRecordFiles.delete(input.dataset.recordUpload);if(message)message.textContent='Archivo no permitido. Usa PDF, JPG, PNG o HEIC de máximo 10 MB.';updateRecordPreview(card,null);updateSaveAllDocuments();return}stagedRecordFiles.set(input.dataset.recordUpload,file);if(message)message.textContent=`Archivo seleccionado: ${file.name}`;updateRecordPreview(card,file);updateSaveAllDocuments()};
