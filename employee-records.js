...
async function uploadEmployeeDocument(input){
  ...
  input.disabled=true;const label=input.closest('.record-upload');label?.classList.add('uploading');
  const uploaded=await appCore.records.upload(path,file);
  if(uploaded.error){input.disabled=false;label?.classList.remove('uploading');toast('No se pudo cargar el archivo: '+uploaded.error.message);return}
