...
$('#employeeRecordDialog').onchange=event=>{const input=event.target.closest('[data-record-upload]');if(!input)return;const file=input.files?.[0],card=input.closest('.record-document-card'),button=card?.querySelector('[data-save-document]'),message=card?.querySelector('[data-selected-file]');if(!file){if(button)button.disabled=true;if(message)message.textContent='Ningún archivo seleccionado.';updateRecordPreview(card,null);return}if(button)button.disabled=false;if(message)message.textContent=`Archivo seleccionado: ${file.name}`;updateRecordPreview(card,file)};
