(function(global){
  ...
  function createLocalPreviewPorts(storage,matrixKey,adminKey,snapshotUrl,matrixSnapshotUrl){
    ...
    return {auth:{getVerifiedUser:()=>Promise.resolve({user:{id:'local',email:'entorno.local@skinklinik'},error:null}),signIn:()=>Promise.resolve({user:{id:'local',email:'entorno.local@skinklinik'},error:null}),signOut:()=>ok(true)},matrix:{load:()=>ok({datos:matrixState.load()}),save:()=>ok({local:true})},admin:{loadSummary:async()=>ok(await loadAdmin()),loadEmployeeDetails:()=>ok([]),loadRegional,loadGeneral,saveEmployee,setEmployeePin,verifyPin,saveCatalog},access,localState:matrixState};
  }
